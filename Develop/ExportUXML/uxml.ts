import { Bounds, SceneNode } from "scenegraph";
...
// 全体にかけるスケール
let globalScale: number = 1.0

export let globalCssRules = null

export let globalCssVars = {}

// コンバート中のRoot
export let globalRootNode: SceneNode = null

export let globalCacheNodeNameAndStyle = {}

// レスポンシブパラメータを保存する
export let globalResponsiveBounds: BoundsToRectTransform[] = null

export let globalCacheParseNodeName = {}
...
function resetGlobalVars()
{
  ...
  globalCssVars = {}
  globalRootNode = null
  globalCacheNodeNameAndStyle = {}
  globalResponsiveBounds = null
  globalCacheParseNodeName = {}
}
 ...
 */
function getGlobalBounds(node): BoundsE {
 ...
 */
function getGlobalDrawBounds(node): BoundsE {
 ...
 * @return {{ex: number, ey: number, x: number, width: number, y: number, height: number}}
 */
function getBeforeGlobalBounds(node): BoundsE {
...
class BoundsToRectTransform {
  ...
  private responsiveParameter: RectTransform
  private responsiveParameterGlobal: RectTransform
...
export class UXMLGenerator {
  ...
  addStyleHeight(json, node: SceneNode) {
    Object.assign(json.attributes, {
      style: 'height: 233px;',
    })
  }

  async traverse(json, node: SceneNode) {
    const result = true
  ...
  async traverseChild(json, node: SceneNode) {
  ...
  async generate(rootNode: SceneNode) {
    resetGlobalVars()
