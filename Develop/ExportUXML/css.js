...
class CssSelector {
    ...
    matchRule(node, rule = null, verboseLog = false) {
        ...
        if (rule.nestingOperator === '>' || rule.nestingOperator == null) {
            if (verboseLog)
                console.log(`nestingオペレータ${rule.nestingOperator} 確認のため、checkNodeを親にすすめる`);
            checkNode = checkNode.parent;
        }
        return checkNode;
    }
    ...
    static check(node, rule, verboseLog = false) {
        ...
        if (rule.pseudos) {
            if (verboseLog)
                console.log('## 疑似クラスチェック');
            for (let pseudo of rule.pseudos) {
                if (!this.checkPseudo(node, pseudo)) {
                    if (verboseLog)
                        console.log(`- ${pseudo.name} が適合しません`);
                    return false;
                }
                if (verboseLog)
                    console.log(`- ${pseudo.name} が適合した`);
            }
        }
        //console.log(nodeName)
        //console.log(JSON.stringify(parsedNodeName, null, '  '))
        if (verboseLog)
            console.log('このruleは適合した');
        return true;
    }
    static checkAttr(node, parsedNodeName, attr) {
        switch (attr.name) {
            case 'class': {
                if (!CssSelector.namesCheck(attr.operator, parsedNodeName.classNames, attr.value))
                    return false;
                break;
            }
            case 'id': {
                if (!CssSelector.nameCheck(attr.operator, parsedNodeName.id, attr.value))
                    return false;
                break;
            }
            case 'tag-name': {
                if (!CssSelector.nameCheck(attr.operator, parsedNodeName.tagName, attr.value))
                    return false;
                break;
            }
            case 'type-of':
            case 'typeof': {
                if (!CssSelector.nameCheck(attr.operator, node.constructor.name, attr.value))
                    return false;
                break;
            }
            default:
    ...
    static checkPseudo(node, pseudo) {
        ...
        switch (pseudo.name) {
            case 'nth-child':
                ...
                const nodeChildIndex = node_1.getChildIndex(node) + 1;
                result = nthChild === nodeChildIndex;
                break;
            case 'first-child':
                result = node_1.isFirstChild(node);
                break;
            case 'last-child':
                result = node_1.isLastChild(node);
                break;
            case 'only-child':
                result = node_1.isOnlyChild(node);
                break;
    ...
    static namesCheck(op, names, value) {
        ...
        for (let name of names) {
            if (this.nameCheck(op, name, value))
                return true;
        }
        return false;
    }
...
class CssDeclarations {
    ...
    firstAsBool(property) {
        return tools_1.asBool(this.first(property));
    }
}
    ...
    while ((token = tokenizer.exec(text))) {
        try {
            ...
            if (errorThrow) {
                ...
                const errorText = `CSSのパースに失敗した: ${lines.length}行目:${errorLine}\n` +
                    e.message;
...
exports.loadCssRules = loadCssRules;
